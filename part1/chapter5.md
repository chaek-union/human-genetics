...
![The bottleneck of Sanger sequencing](../assets/figures/ch5_fig1.png)

**Figure 5-1. The Bottleneck of Sanger Sequencing**. *Sanger sequencing processes DNA fragments sequentially, one at a time. This serial approach created a major bottleneck for large-scale genomic studies. While a single fragment can be read accurately, sequencing an entire genome requires processing billions of fragments in series, making the process extremely time-consuming and expensive.*

![Sanger sequencing method](https://microbenotes.com/wp-content/uploads/2023/02/Sanger-Sequencing.jpg)

**Figure 5-2. Sanger Sequencing Method**. *This diagram illustrates how Sanger sequencing works. Source: [Microbe Notes](https://microbenotes.com/illumina-sequencing/)*

Imagine you're trying to transcribe a million books, but you can only read and copy one sentence at a time before moving to the next. That's what Sanger sequencing does. For the Human Genome Project, this meant reading 3 billion letters of DNA code one fragment at a time. Even with hundreds of sequencing machines running 24/7, it took years.

After the HGP, it became clear that genomics couldn't progress at this pace. Scientists wanted to sequence not just one genome, but thousands comparing genomes from healthy and sick individuals, studying genetic variation across populations, identifying disease-causing mutations in patients. At $300 million and a decade per genome, this wasn't feasible.
...
![Massive parallel sequencing concept](../assets/figures/ch5_fig3.png)

**Figure 5-3. The Core Innovation of NGS: Massive Parallelization**. *The fundamental difference between Sanger sequencing and NGS is parallelization. While Sanger sequencing processes fragments one at a time (like one person copying a book), NGS sequences millions of fragments simultaneously (like millions of people copying different parts at once). This parallel approach dramatically reduced sequencing time from years to days.*
...
**Figure 5-4. Reads, Coverage, and Depth Explained**. *Coverage refers to what proportion of the genome has been sequenced at least once. Depth indicates how many times each position was sequenced on average. Higher depth provides greater confidence in distinguishing real variants from sequencing errors.*
...
![Why read length matters](../assets/figures/ch5_fig5.png)

**Figure 5-5. Why Read Length Matters**. *Short reads struggle to span repetitive genomic regions, leaving gaps and ambiguities in assembly. Long reads can span entire repetitive regions in a single read, providing unambiguous sequence information. This difference is critical for resolving complex genomic structures like centromeres, telomeres, and segmental duplications.*
...
Illumina is the standard choice for most genomics applications: whole-genome sequencing (WGS), whole-exome sequencing (WES sequencing just the protein-coding regions), identifying SNPs and small indels, RNA sequencing, and clinical diagnostics. For sequencing many samples affordably and accurately, Illumina remains the primary option.
...
**Figure 5-7. PacBio Single-Molecule Real-Time (SMRT) Sequencing**. *This diagram shows the key components of PacBio sequencing. A DNA polymerase is fixed at the bottom of a zero-mode waveguide (ZMW) a tiny well that allows detection of just the molecules at the bottom. As the polymerase incorporates fluorescently labeled nucleotides into the growing DNA strand, brief flashes of light are detected and recorded in real time. The circular DNA template (SMRTbell) allows the polymerase to sequence the same molecule multiple times, improving accuracy. Source: [Microbe Notes](https://microbenotes.com/pacbio-sequencing/)*
...
A notable early example: in 2010, researchers used WES to identify the cause of Miller syndrome in just four siblings by comparing their exomes to their healthy parents. The gene **DHODH** had never been linked to the disease before. What might have taken decades with traditional methods took months with NGS.
...
**Figure 5-9. The Cost of Sequencing a Human Genome Over Time**. *This graph from the National Human Genome Research Institute shows the dramatic decrease in sequencing costs from 2001 to 2022. The blue line shows the actual cost per genome, while the dotted line represents Moore's Law (the rate at which computer technology improves). Notice that after 2008, when NGS technologies became widely available, sequencing costs dropped far faster than Moore's Law a remarkable achievement. The vertical axis uses a logarithmic scale, so each step down represents a 10-fold cost reduction. Source: [NHGRI Genome Sequencing Program](https://www.genome.gov/about-genomics/fact-sheets/DNA-Sequencing-Costs-Data)*
